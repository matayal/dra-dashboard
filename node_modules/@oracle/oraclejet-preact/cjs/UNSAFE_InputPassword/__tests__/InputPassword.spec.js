...
describe('Test InputPassword component', () => {
    describe('Test InputPassword translation bundle', () => {
        it('render', async () => {
            const env = await getEnvironmentContext();
            const content = preact.render(jsxRuntime.jsx(UNSAFE_Environment.RootEnvironmentProvider, Object.assign({ environment: env }, { children: jsxRuntime.jsx(UNSAFE_InputPassword.InputPassword, { label: "Test Label", onInput: noOp }) })));
            const revealButton = content.container.querySelector('button');
            chai.expect(revealButton.getAttribute('aria-label')).equals('Password Hidden');
        });
    });
});
/*  */
