...
const FocusTrap = ({
  ...
  const lastActiveElementRef = compat.useRef(null); // classes to be set on the children wrapper
  ...
  const handleStartGuardOnFocus = event => {
    ...
    utils_PRIVATE_tabbableUtils.focusOnEnd(trapRef.current); // prevent scroll jump and bubbling this event up as a focus event
  ...
  const handleEndGuardOnFocus = event => {
    if (isDisabled || !trapRef.current) return;
    utils_PRIVATE_tabbableUtils.focusOnStart(trapRef.current); // prevent scroll jump and bubbling guard event up as a focus event
  ...
  compat.useEffect(() => {
    ...
    const originalActiveElement = utils_PRIVATE_tabbableUtils.getActiveElement(trap); // init focus
...
const hasFocus = (elements, target) => {
  ...
  const activeElement = target !== null && target !== void 0 ? target : utils_PRIVATE_tabbableUtils.getActiveElement(elements[0]);
...
/*  */
//# sourceMappingURL=FocusTrap-8af50913.js.map
