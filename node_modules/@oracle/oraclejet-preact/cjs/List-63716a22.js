...
var LoadMoreCollection = require('./LoadMoreCollection-eaa4ed6f.js');
...
function ListViewItem({
  ...
  const roles = hooks.useMemo(() => getListViewRoles(role), [role]); // some roles come with related aria attributes which need to be set
  ...
  return jsxRuntime.jsx("div", Object.assign({
    ...
    children: jsxRuntime.jsx("div", Object.assign({
      ...
      children: jsxRuntime.jsx(hooks_UNSAFE_useTabbableMode.TabbableModeContext.Provider, Object.assign({
        value: {
          isTabbable
        }
      }, {
        children: jsxRuntime.jsx(FocusTrap.FocusTrap, Object.assign({
...
function List({
  ...
  const rootRef = hooks.useRef(null);
  const listRoles = hooks.useMemo(() => getListViewRoles(role), [role]); // tracking the anchor key which is used for shift+click selection

  const anchorKey = hooks.useRef(); // tracking pending selection/current key that is not in current data

  const pendingSelection = hooks.useRef();
  const pendingCurrentKey = hooks.useRef(); // tracking sparkle height
  ...
  const {
    currentKeyProps
  } = hooks_PRIVATE_useCurrentKey.useCurrentKey(element => utils_PRIVATE_collectionUtils.keyExtractor(element, ITEM_SELECTOR), utils_PRIVATE_collectionUtils.getPrevNextKey(rootRef.current, currentKey, true, ITEM_SELECTOR), utils_PRIVATE_collectionUtils.getPrevNextKey(rootRef.current, currentKey, false, ITEM_SELECTOR), currentKey, onCurrentKeyChange);
  ...
  const [showFocusRing, focusRingProps] = hooks_PRIVATE_useCollectionFocusRing.useCollectionFocusRing(listViewContains, ['ArrowUp', 'ArrowDown']);
  let pendingFocusRingChange = false;
  const [isCurrentTabbableKey, tabbableModeProps] = hooks_PRIVATE_useTabbableModeSet.useTabbableModeSet(listViewContains, element => utils_PRIVATE_collectionUtils.keyExtractor(element, ITEM_SELECTOR), currentKey, // handle clean-up on disabling of the mode
  ...
  const {
    selectionProps
  } = hooks_PRIVATE_useSelection.useSelection(element => {
    if (element === rootRef.current) {
      return currentKey === undefined ? null : currentKey;
    }

    return utils_PRIVATE_collectionUtils.keyExtractor(element, ITEM_SELECTOR);
  }, selectedKeys, selectionMode, false, 'replace', handleSelectionChange, anchorKey.current, currentKey, (currentKey, isPrev) => utils_PRIVATE_collectionUtils.getPrevNextKey(rootRef.current, currentKey, isPrev, ITEM_SELECTOR), key => {
    if (rootRef.current) {
  ...
  const suggestions = hooks.useMemo(() => findSuggestions(data), [data]); // TODO: check suggestions before Sparkle would cause a scrolling issue

  const sparkleIndicator = jsxRuntime.jsx(Sparkle, {
    sparkleHeight: sparkleHeight
  });

  const itemActionProps = hooks_PRIVATE_useItemAction.useItemAction(currentKey, data, onItemAction, ITEM_SELECTOR);
  hooks.useEffect(() => {
  ...
  const getItemContext = hooks.useCallback(context => {
    const selectorRenderer = selectionMode === 'multiple' ? () => jsxRuntime.jsx(UNSAFE_Selector.Selector, {
  ...
  const collectionComponent = isVirtualized ? jsxRuntime.jsx(VirtualizedCollection.VirtualizedCollection, Object.assign({
    data: data,
  ...
  })) : jsxRuntime.jsx(LoadMoreCollection.LoadMoreCollection, Object.assign({
    data: data,
    loadMoreIndicator: defaultLoadMoreIndicator,
    onLoadRange: onLoadRange,
    suggestions: sparkleIndicator,
  ...
  const handlePointerDown = hooks.useCallback(event => {
    const key = utils_PRIVATE_collectionUtils.keyExtractor(event.target, ITEM_SELECTOR);
    if (utils_UNSAFE_keys.isKeyDefined(key)) pendingCurrentKey.current = key;
  ...
  return jsxRuntime.jsx("div", Object.assign({}, utils_UNSAFE_mergeProps.mergeProps(allowTabbableMode ? tabbableModeProps : {}, currentKeyProps, focusRingProps, selectionProps, itemActionProps, {
    onFocus: handleFocus,
...
const defaultLoadingIndicator = jsxRuntime.jsx(SkeletonContainer, Object.assign({
  minimumCount: 25
}, {
  children: () => {
...
const defaultLoadMoreIndicator = jsxRuntime.jsx(SkeletonContainer, Object.assign({
  minimumCount: 3
}, {
  children: () => {
    return jsxRuntime.jsx(Flex.Flex, Object.assign({
...
/*  */
//# sourceMappingURL=List-63716a22.js.map
