...
var hooks_UNSAFE_useBreakpointValues = require('./hooks/UNSAFE_useBreakpointValues.js');
...
function MessageToast({
  ...
  const translations = hooks_UNSAFE_useTranslationBundle.useTranslationBundle('@oracle/oraclejet-preact'); // We need a ref that holds the current & previous data length, as the exiting
  ...
  const prevDataLengthRef = hooks.useRef(0); // Update the data length ref
  ...
  const {
    controller,
    handlers
  } = hooks_PRIVATE_useMessagesFocusManager.useMessageFocusManager(focusHandleRef, {
  ...
  const setHTMLDivElement = hooks.useCallback( // TODO: use ramda.curry here (currently it is throwing ts error when used with generics)
  key => {
  ...
  const maxWidth = hooks_UNSAFE_useBreakpointValues.useBreakpointValues({
  ...
  const paddedOffset = hooks_UNSAFE_useBreakpointValues.useBreakpointValues({
    sm: {
  ...
  const messageTranslations = hooks.useMemo(() => ({
  ...
  const rootClasses = classNames.classNames([messageToastStyles.base, // by default the toast messages are dark, so add the required classes
  ...
  if (!shouldRender && data.length === 0) {
    return jsxRuntime.jsx(UNSAFE_LiveRegion.LiveRegion, {
      children: liveRegionText
    });
  }

  return jsxRuntime.jsxs(UNSAFE_Environment.EnvironmentProvider, Object.assign({
    environment: {
      colorScheme: 'dark'
    }
  }, {
    children: [jsxRuntime.jsx(MessageLayer.MessageLayer, Object.assign({
      offset: paddedOffset,
      position: position
    }, {
          ...
          children: jsxRuntime.jsx(Flex.Flex, Object.assign({
            ...
            children: jsxRuntime.jsx(MessagesManager.MessagesManager, Object.assign({
              ...
              children: ({
                index,
                item
              }) => jsxRuntime.jsx(PRIVATE_Message.Message, {
                messageRef: setHTMLDivElement(item.key),
                item: item,
                detailRenderer: MessageUtils.getRendererWithoutIndex(item, detailRendererKey, renderers),
                iconRenderer: MessageUtils.getRendererWithoutIndex(item, iconRendererKey, renderers),
                index: index,
                onClose: onClose,
    ...
    })), jsxRuntime.jsx(UNSAFE_LiveRegion.LiveRegion, {
      children: liveRegionText
    })]
  }));
}
...
/*  */
//# sourceMappingURL=MessageToast-3ef5b6c9.js.map
