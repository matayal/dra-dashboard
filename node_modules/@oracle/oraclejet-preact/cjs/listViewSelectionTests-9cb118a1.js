...
const testRender = (TestComponent) => {
    ...
    if (typeof jest == typeof undefined) {
        it('initial render with sparkle', async () => {
            ...
            const sparkle = (_b = (_a = grid.firstElementChild) === null || _a === void 0 ? void 0 : _a.firstElementChild) === null || _b === void 0 ? void 0 : _b.firstElementChild;
            chai.expect(sparkle !== undefined).eq(true);
            const items = await testHelpers.getItems(list);
            chai.expect(items === null || items === void 0 ? void 0 : items.length).eq(25);
...
const testAria = (TestComponent) => {
    ...
    it('aria structure and properties (single-select)', async function () {
        ...
        const list = preact.render(jsxRuntime.jsx(TestComponent, { data: dataMetadata, selectionMode: "single" }));
        const grid = await testHelpers.getRoot(list);
        chai.expect(grid.getAttribute('aria-rowcount')).eq('10');
    ...
    it('aria structure and properties (multi-select)', async function () {
        ...
        const list = preact.render(jsxRuntime.jsx(UNSAFE_Environment.RootEnvironmentProvider, Object.assign({ environment: env }, { children: jsxRuntime.jsx(TestComponent, { data: dataMetadata, selectionMode: "multiple" }) })));
        const grid = await testHelpers.getRoot(list);
        chai.expect(grid.getAttribute('aria-rowcount')).eq('10');
...
const testKeys = (TestComponent) => {
    ...
    it('supports currentKey logic with pointerDown', async () => {
        ...
        chai.expect(grid.hasAttribute('aria-activedescendant')).eq(false);
        const items = await testHelpers.getContents(list);
        preact.fireEvent.pointerDown(items[2]);
        preact.waitFor(() => {
...
const testGridlines = (TestComponent) => {
    it('gridlines - gridlines are hidden', async () => {
        ...
        const list = preact.render(jsxRuntime.jsx(TestComponent, { data: dataMetadata }));
        const items = await testHelpers.getItems(list);
        chai.expect(items[0].classList.length).eq(2);
        chai.expect(items[1].classList.length).eq(2);
        chai.expect(items[2].classList.length).eq(2);
    });
    it('gridlines - bottom gridlines are visible', async () => {
        ...
        const list = preact.render(jsxRuntime.jsx(TestComponent, { data: dataMetadata, gridlines: visible }));
        const items = await testHelpers.getItems(list);
        chai.expect(items[0].classList.length).eq(3);
        chai.expect(items[1].classList.length).eq(3);
        chai.expect(items[2].classList.length).eq(3);
    });
    it('gridlines - bottom gridlines are visible except for the last list item', async () => {
        ...
        const list = preact.render(jsxRuntime.jsx(TestComponent, { data: dataMetadata, gridlines: visibleExceptLast }));
        const items = await testHelpers.getItems(list);
        chai.expect(items[0].classList.length).eq(3);
        chai.expect(items[1].classList.length).eq(3);
        chai.expect(items[2].classList.length).eq(2);
    ...
    it('gridlines - bottom gridlines are visible; The top gridlines of the first list item is visible', async () => {
        ...
        const list = preact.render(jsxRuntime.jsx(TestComponent, { data: dataMetadata, gridlines: visibleWithFirst }));
        const items = await testHelpers.getItems(list);
        chai.expect(items[0].classList.length).eq(4);
        chai.expect(items[1].classList.length).eq(3);
        chai.expect(items[2].classList.length).eq(3);
    });
    it('gridlines - bottom gridlines are visible except for the last list item. The top gridlines of the first list item is visible.', async () => {
        ...
        const list = preact.render(jsxRuntime.jsx(TestComponent, { data: dataMetadata, gridlines: visibleWithFirstButNotLast }));
        const items = await testHelpers.getItems(list);
        chai.expect(items[0].classList.length).eq(4);
        chai.expect(items[1].classList.length).eq(3);
        chai.expect(items[2].classList.length).eq(2);
...
const testItemAction = (TestComponent) => {
    it('ItemAction - test click on item', async () => {
        ...
        const onItemAction = (detail) => {
            if (count === 0) {
                chai.expect(detail.reason, 'Reason is click').eq('click');
        ...
        const list = preact.render(jsxRuntime.jsx(TestComponent, { data: dataMetadata, onItemAction: onItemAction }));
    ...
    it('ItemAction - test press enter on item', async () => {
        ...
        const onItemAction = (detail) => {
            if (count === 1) {
                chai.expect(detail.reason, 'Reason is enter').eq('enter');
                chai.expect(detail.context.data, 'Context data is correct').eq(dataMetadata[1].data);
                chai.expect(detail.context.metadata, 'Context metadata is correct').eq(dataMetadata[1].metadata);
            }
            else if (count === 2) {
    ...
    it('ItemAction - test press spacebar on item', async () => {
        ...
        const onItemAction = (detail) => {
            if (count === 1) {
                chai.expect(detail.reason, 'Reason is spacebar').eq('spacebar');
                chai.expect(detail.context.data, 'Context data is correct').eq(dataMetadata[1].data);
                chai.expect(detail.context.metadata, 'Context metadata is correct').eq(dataMetadata[1].metadata);
            }
            else if (count === 2) {
    ...
    it('currentItemVariant - highlight', async () => {
        ...
        const list = preact.render(jsxRuntime.jsx(TestComponent, { data: dataMetadata, initialKey: dataMetadata[1].metadata.key, currentItemVariant: "highlight" }));
        const items = await testHelpers.getItems(list);
        chai.expect(items[0].classList.length).eq(2);
        chai.expect(items[1].classList.length).eq(3);
        chai.expect(items[2].classList.length).eq(2);
    ...
    it('currentItemVariant - none', async () => {
        ...
        const list = preact.render(jsxRuntime.jsx(TestComponent, { data: dataMetadata, initialKey: dataMetadata[1].metadata.key, currentItemVariant: "none" }));
        const items = await testHelpers.getItems(list);
        chai.expect(items[0].classList.length).eq(2);
        chai.expect(items[1].classList.length).eq(2);
        chai.expect(items[2].classList.length).eq(2);
...
const testSelection = (TestComponent) => {
    it('select single - numeric key', async () => {
        ...
        const list = preact.render(jsxRuntime.jsx(TestComponent, { data: dataMetadata, selectionMode: "single", onClick: () => {
    ...
    it('selects multiple items when clicking on Selector checkboxes', async () => {
        ...
        const list = preact.render(jsxRuntime.jsx(UNSAFE_Environment.RootEnvironmentProvider, Object.assign({ environment: env }, { children: jsxRuntime.jsx(TestComponent, { data: dataMetadata, selectionMode: "multiple" }) })));
        const items = await testHelpers.getContents(list);
        testHelpers.mouseCheckboxSelection(items[2]);
        await preact.waitFor(() => {
            chai.expect(items[2].getAttribute('aria-selected')).eq('true');
            chai.expect(items[4].getAttribute('aria-selected')).eq('false');
        });
        testHelpers.mouseCheckboxSelection(items[4]);
        await preact.waitFor(() => {
            chai.expect(items[2].getAttribute('aria-selected')).eq('true');
            chai.expect(items[4].getAttribute('aria-selected')).eq('true');
        });
...
/*  */
//# sourceMappingURL=listViewSelectionTests-9cb118a1.js.map
