...
function SelectSingleMenuGroup({ value, onCommit, children }) {
    return (jsxRuntime.jsx(SelectMenuItem.SelectMenuGroupContext.Provider, Object.assign({ value: { isMultiple: false, value, onCommit } }, { children: children })));
}

const SelectMenu = ({ initialValue, onCommit }) => {
    const [selectSingleValue, setSelectSingleValue] = hooks.useState(initialValue);
    const handleCommit = ({ value }) => {
        setSelectSingleValue(value);
    };
    const ref = hooks.useRef(null);
    return (jsxRuntime.jsx(jsxRuntime.Fragment, { children: jsxRuntime.jsxs(UNSAFE_Environment.RootEnvironmentProvider, { children: [jsxRuntime.jsx(Button.Button, { ref: ref, label: "Text Wrapping" }), jsxRuntime.jsx(Menu.Menu, Object.assign({ isOpen: true, anchorRef: ref }, { children: jsxRuntime.jsxs(SelectSingleMenuGroup, Object.assign({ value: selectSingleValue, onCommit: onCommit || handleCommit }, { children: [jsxRuntime.jsx(SelectMenuItem.SelectMenuItem, { value: "overflow", label: "Overflow" }), jsxRuntime.jsx(SelectMenuItem.SelectMenuItem, { value: "wrap", label: "Wrap" }), jsxRuntime.jsx(SelectMenuItem.SelectMenuItem, { value: "clip", label: "Clip" })] })) }))] }) }));
};
describe('Test Menu Component', () => {
    ...
    it('onCommit called when selecting item', async function () {
        ...
        const menuItems = menuTestsUtils.getAllMenuItemsByRole(content, 'menuitemradio');
    ...
    it('Initial SelectSingleMenuGroup render with value', async function () {
        const content = preact.render(jsxRuntime.jsx(SelectMenu, { initialValue: "overflow" }));
        const menuItems = menuTestsUtils.getAllMenuItemsByRole(content, 'menuitemradio');
        const firstItem = menuItems[0];
        const secondItem = menuItems[1];
        const thirdItem = menuItems[2];
    ...
    it('SelectSingleMenuGroup remains the same if selected item is selected again', async function () {
        ...
        const menuItems = menuTestsUtils.getAllMenuItemsByRole(content, 'menuitemradio');
        preact.fireEvent.click(menuItems[0]);
        const menuItemsAfterClick = menuTestsUtils.getAllMenuItemsByRole(content, 'menuitemradio');
